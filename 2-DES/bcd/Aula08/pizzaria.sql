...
-- Insert com select para pegar o valor da Pizzas e copiar para o valor do ítem Pedidos
...
create procedure insere_pedidos()
    ...
    start transaction;
        insert into Pedidos(cliente_id, data, hora) values(30,DATE_SUB(curdate(),INTERVAL 10 DAY),"18:54:00");
        insert into Itens_Pedido(pedido_id, pizza_id, quantidade, valor) values(30,2,1, (select valor from Pizzas where pizza_id = 2));
        insert into Itens_Pedido(pedido_id, pizza_id, quantidade, valor) values(30,4,2, (select valor from Pizzas where pizza_id = 4));
        insert into Itens_Pedido(pedido_id, pizza_id, quantidade, valor) values(30,6,3, (select valor from Pizzas where pizza_id = 6));
        ...
        if erro_sql = false then
            ...
            rollback;
            select 'Erro no Pedido' as Resultado;
        end if;
end//
delimiter ;
call insere_pedidos();
